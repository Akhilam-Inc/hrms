<template>
    <div class=" sm:w-96 w-full flex flex-col h-screen justify-between relative">
        <div class="">
            ...
            <span class=" font-bold text-[19px] px-2">New Sales Visit</span>
            </div>
            <div class="px-5 mt-7">
                <div class="">
                    ...
                    <div class="mt-7">
                        ...
                        <div v-if="selectedFile.length" class="w-full mt-4">
                            <ul class="w-full flex fle-col items-center gap-2 mt-2"  v-for="file in selectedFile">
...
function handleFileDelete(fileobj){
    let index = selectedFile.value.indexOf(fileobj)
...
function handleFileSelect(ev) {
    ...
    reader.onload = (e) => {
        ...
        selectedFile.value.push([ev.target.files, result.value])
...
const uploader = createResource({
        url: "saleswise.api.upload_base64_file"
})
...
function folloup(){
    ...
    window.location.href = `${origin}/saleswise/follow-up/${doc}/${endpoint}`
}
onMounted(() => {
